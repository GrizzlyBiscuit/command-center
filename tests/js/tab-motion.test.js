...
test("immersive palettes have distinct reduced-motion-safe tab entrances", () => {
  const expected = {
    ...
    tropical: "cc-tropical-tab-in 360ms",
  };
  ...
  assert.match(modern, /@keyframes\s+cc-tropical-tab-in\b/);
  ...
  assert.match(modern, /html\[data-theme="tropical"\] body::before \{[^}]*animation:\s*cc-tropical-surf 7s ease-in-out infinite alternate;/);
  ...
  assert.match(modern, /@media \(prefers-reduced-motion: reduce\)[\s\S]*?html\[data-theme="tropical"\] body::before,[\s\S]*?html\[data-theme="tropical"\] \.cc-music-now-playing::after,[\s\S]*?animation:\s*none !important;/);
});
