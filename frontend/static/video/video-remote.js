/*
 * Command Center video remote bridge.
 * Adapted from the Command Center music bridge. SPDX-License-Identifier: MIT.
 */
(function (root, factory) {
  "use strict";
  const api = factory();
  if (typeof module === "object" && module.exports) module.exports = api;
  if (root) root.CCVideoRemote = Object.freeze(api);
})(typeof window !== "undefined" ? window : globalThis, function () {
  "use strict";

  const OUTPUT_KEY = "cc.video.output.v1";
  const RENDERER_KEY = "cc.video.renderer.v1";
  const OUTPUT_DEVICE = "device";
  const OUTPUT_COMPUTER = "computer";
  const MAX_QUEUE_VIDEOS = 1000;
  ...
  function boundQueueState(values, requestedIndex = -1, limit = MAX_QUEUE_VIDEOS) {
    const queue = (Array.isArray(values) ? values : [])
      .map(value => String(value || "").trim())
      .filter(Boolean);
    const maximum = Math.max(1, Math.trunc(finite(limit, MAX_QUEUE_VIDEOS)));
  ...
  function filterQueueState(values, requestedIndex, isKnown = () => true) {
    const source = (Array.isArray(values) ? values : []).map(value => String(value || "").trim());
  ...
  function createBridge(options = {}) {
    ...
    function setRendererConnected(value) {
      ...
      options.onRendererConnectionChange?.(next);
    }
    ...
    async function heartbeat() {
      ...
      const response = await options.request("/api/video/remote/renderer", {
        method: "POST",
        body: { renderer_id: rendererId, epoch, ack: acknowledged, state: rendererState },
      });
      ...
      if (response?.renderer === false || response?.claimed === false) {
        throw new Error(response?.error || "Another Command Center PC video player is active.");
      }
    ...
    async function refresh() {
      ...
      const query = snapshot.epoch
        ...
        : "";
      const payload = await options.request(`/api/video/remote${query}`);
      if (generation !== refreshGeneration || pendingCommands > 0) return snapshot;
    ...
    function enqueueCommand(payload, { allowLeaving = false } = {}) {
      ...
      const execute = async () => {
        try {
          ...
          const response = await options.request("/api/video/remote/command", {
            method: "POST",
            body: payload,
          });
